...
<div class="form">
  <a href="board.php"> 게시판</a> > 공지사항 및 이벤트 검색
  <span class="search"><i class="xi-search"></i>
  <input type="search" name="search1" onsearch="javascript:e_search()">
  </span><br>
  </div>
        <?php
            $search=$_GET["search"];
            include "conn.php";
            $sql="select * from board where title like '%$search%' union select * from board where content like '%$search%'" ;
            $rs=mysqli_query($conn,$sql);
